Ultra_collection_namespace.Ultra_collection Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: ultra_collection_namespace.ultra_collection.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from ultra_collection_namespace.ultra_collection
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: ultra_collection_namespace.ultra_collection.run
      vars:
        run_x: 42
```
